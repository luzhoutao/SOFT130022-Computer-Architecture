...
/* create a flat memory space in shared memory*/
struct mem_t *
mem_shm_create(char *name, key_t key, int *shmid)			/* name of the memory space */
{
  struct mem_t *mem;

  mem = (struct mem_t *)Shmalloc(key, sizeof(struct mem_t), shmid);
  if (!mem)
    fatal("out of virtual memory");
  char *temp = mystrdup(name);
  printf("name: %x\n", temp);
  mem->name = temp;
	    ...
	    md_addr_t addr)		/* virtual address to allocate */
{
  ...
  //page = (byte_t *)mem_get_new_shm(MD_PAGE_SIZE);
  ...
  /* generate a new PTE */
  //pte = (struct mem_pte_t *) mem_get_new_shm(sizeof(struct mem_pte_t));
