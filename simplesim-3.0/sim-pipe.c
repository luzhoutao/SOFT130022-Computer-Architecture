...
if(!enable_icache){
	...
	//printf("have instruction cache.\n");
	lat = ucache_access(cp, Read, CPC, (word_t *) &instruction, 2);
	//printf("addr:%x instr:%x %x\n", CPC, instruction.a, instruction.b);
}
...
/**if(de.opcode==LW){
  ...
  if(de.opcode == MULTU){
    ...
    for (_i = 0; _i < 31; _i++){									
	    ...
	    SET_LO(LO << 1);					
	    if ((extractl(data2, 30 - _i, 1)) == 1){								
	        if (((unsigned)037777777777 - (unsigned)LO) < (unsigned)(de.data1)){		
		        SET_HI(HI + 1);						 
	        }								
	        SET_LO(LO + de.data1);					
	    }								
...
void do_mem()
{
  ...
  if(em.mem_signal.writeMem == 1){
	int lat;
if(!enable_dcache){
    enum md_fault_type _fault;fprintf(trace, "%x\n", em.ALUres);
    WRITE_WORD((word_t) em.data2, em.ALUres, _fault);			
    if (_fault != md_fault_none)
      DECLARE_FAULT(_fault);
	lat = PENALTY;
}else{
	lat = ucache_access(cp, Write, em.ALUres, (word_t *) &em.data2, 1);
}
	for(;lat>0;lat--)
		CLOCK();
  }
...
if(!enable_dcache){
	...
	lat = ucache_access(cp, Read, em.ALUres, (word_t *) &mw.memData, 1);
}
	for(;lat>0;lat--)
		CLOCK();
  }
