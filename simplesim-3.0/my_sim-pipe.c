...
void do_if()
{
  md_inst_t instruction;
    ...
    lat = cache_access(cp, Read, CPC, &instruction);
...
void do_mem()
{
  ...
  if(em.mem_signal.writeMem == 1){
	...
	lat = 9;
#else
	lat = cache_access(cp, Write, em.ALUres, (word_t *) &em.data2);
#endif
	for(;lat>0;lat--)
		CLOCK();
  }
  ...
  if(em.mem_signal.readMem == 1){
	...
	lat = 9;
#else
	lat = cache_access(cp, Read, em.ALUres, (word_t *) &mw.memData);
#endif
	for(;lat>0;lat--)
		CLOCK();
  }
...
/**void print_line(cache_line_t *line){
...
void print_cache(cache_t *cp){
	printf("access %d, miss %d, hit %d, replace %d, write back %d\n", cp->access, cp->miss, cp->hit, cp->replace, cp->write_back);
	for(int i = 0; i<16; i++){
		printf("set %d----------------------\n", i+1);
		cache_set_t set = cp->sets[i];
		for(int j = 0; j<4; j++){
...
int cache_access(cache_t *cp, enum mem_cmd cmd, md_addr_t addr, word_t *data){	
	/**if(cp->access == 7)
		exit(0);
 ...
 FIND_FREE:
	...
	//do the command
	if(cmd == Read)
		memcpy(data, line->data+block, sizeof(word_t));
	else if(cmd == Write){
